...
include"connect.php";
$err = []; 
if(!isset($_POST['soLuong'])&&empty($err))unset($_SESSION['sanphamnhap']);
if(!isset($_SESSION['sanphamnhap'])) $_SESSION['sanphamnhap']=[];
...
{   
  ...
  if($_POST['btnSubmit'] == 'Thêm sản phẩm') 
  {
    ...
    if($l=='Hàng Thường'){
        ...
        for($i=0;$i<sizeof($_SESSION['sanphamnhap']);$i++){
            if($_SESSION['sanphamnhap'][$i][0]==$maSP){
                $_SESSION['sanphamnhap'][$i][3]+=$sl;
                $t=1;
                break;
            }
        }
 ...
 if($_POST['btnSubmit'] == 'Tạo phiếu') 
  {   
    $sql1 = "select * from hoadonnhaphang where maHoaDonNhap = '$m'";
    $rs1 = mysqli_query($conn,$sql1);
    if(mysqli_num_rows($rs1)>0){
        $err['MPtontai'] = 'Mã phiếu nhập đã tồn tại';
    }
    if(empty($ten)){
    ...
    if(empty($err)){
      $sql0 = "INSERT INTO `hoadonnhaphang` (`maHoaDonNhap`, `loaiNhap`, `maNhanVien`, `maKho`, `nguoiGiao`, `diaChiNhap`, `sdtNoiNhap`) 
      VALUES ('$m',  '$l', '$nv', '$mk', '$ten', '$dc', '$sdt');";
      $query = mysqli_query($conn,$sql0);
      for($i=0; $i<sizeof($_SESSION['sanphamnhap']);$i++){
        $sp=$_SESSION['sanphamnhap'][$i][0];
        $sl=$_SESSION['sanphamnhap'][$i][2];
        $dg=$_SESSION['sanphamnhap'][$i][3];
        $sql5 = "INSERT INTO `sanphamnhap` (`maHoaDonNhap`,`maSanPham`, `soLuongnhap`, `donGianhap`) 
        VALUES ($m, $sp, $sl, $dg);";
        ...
        if($l=='Hàng Thường'){
          if(mysqli_num_rows($rs3)==0)
            mysqli_query($conn,"INSERT INTO `sanpham_kho` (`maKho`,`maSanPham`, `soLuongCon`, `soLuongLoi`) 
            VALUES ($mk, $sp, $sl, 0);");
          else
      ...
      header("Location: TraCuuPhieuNhap.php");
    }
...
<head>
  ...
  <title>Didongthongminh - Quản Lý Nhập</title>
      ...
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
             ...
             <li class="breadcrumb-item active"><a href="./TraCuuPhieuNhap.php">Quản lý phiếu nhập</a></li>
        ...
        <div class="card card-default">
          <div class="card-header">
            <h1 class="card-title">Thêm Phiếu Nhập</h1>
            ...
            <div class="row">
              <div class="col-md-6" data-select2-id="41">
                  <div class="form-group">
                    ...
                    <input name= "maphieu" type="text" class="form-control" placeholder="Mã phiếu nhập..." value="<?php if(!empty($m))echo $m ?>"/>
                    <div style="color:red;">
                  ...
                  <div class="form-group">
                    ...
                    <?php 
                    if (empty($_SESSION['sanphamnhap'])){
                      echo '
                      <select name = "loaiNhap" class="form-control select2 select2-hidden-accessible" style="width: 100%;" data-select2-id="1" tabindex="-1" aria-hidden="true">
                        <option data-select2-id="42">Hàng Thường</option>
                        <option data-select2-id="43">Hàng Lỗi</option>
                      </select>';}
                    else {
                      echo'<input name="loaiNhap" readonly="readonly" class="form-control select2 select2-hidden-accessible" type="text" value="'.$l.'"/>';
                    }
                    ?>
                  </div>
                  <div class="form-group">
                    <label>Tên người giao</label>
                    <input name='benGiao' class="form-control select2 select2-hidden-accessible" type="text" placeholder="Nhập tên..." value="<?php if(isset($m))echo $ten ?>">
                    <div style="color: red">
